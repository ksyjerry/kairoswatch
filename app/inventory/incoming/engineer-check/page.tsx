...
import EngineerInspectionModal from "@/components/engineer-inspection-modal"
...
export default function EngineerCheckPage() {
  ...
  const filteredInvoices = invoiceData.filter((invoice) => {
    ...
    const matchesStatus = statusFilter === "all" || invoice.engineerStatus === statusFilter

    // 영업팀 검수가 완료된 항목만 표시 (영업팀 검수 중인 항목은 제외)
    const isSalesCheckCompleted = invoice.salesStatus === "검수 완료"

    return matchesSearch && matchesStatus && isSalesCheckCompleted
  })
  ...
  return (
    <>
      <div className="mb-6">
        <h1 className="text-2xl font-medium">엔지니어팀 검수</h1>
        <p className="text-sm text-[#5b5c5b] mt-1">입고된 제품의 기술적 검수를 진행합니다.</p>
      </div>
          ...
          <table className="w-full min-w-[768px]">
            <thead className="bg-[#f7f9f8]">
              <tr>
                ...
                <th className="font-medium text-[#1f1f1f] py-3 px-4 text-center">영업팀</th>
                <th className="font-medium text-[#1f1f1f] py-3 px-4 text-center">엔지니어팀</th>
              ...
              {filteredInvoices.map((invoice) => (
                <tr key={invoice.id} className="hover:bg-[#f7f9f8] cursor-pointer">
                  ...
                  <td className="py-3 px-4 border-t border-[#dde0df] text-center">
                    <span
                      className={`inline-block px-2 py-1 rounded-full text-xs ${getStatusBadgeStyle(
                        invoice.salesStatus,
                      )}`}
                    >
                      {invoice.salesStatus}
                  ...
                  <td className="py-3 px-4 border-t border-[#dde0df] text-center">
                    <button
                      ...
                      disabled={invoice.salesStatus !== "검수 완료"}
                    >
                      검수하기
                    </button>
                  </td>
        ...
        {filteredInvoices.map((invoice) => (
          ...
          >
            <div className="flex justify-between items-center mb-2">
              ...
              <span className={`px-2 py-1 rounded-full text-xs ${getStatusBadgeStyle(invoice.engineerStatus)}`}>
                {invoice.engineerStatus}
              </span>
            </div>
            ...
            <div className="flex justify-between items-center mb-3">
              <div className="flex items-center">
                <span className="text-xs text-[#5b5c5b] mr-2">영업팀:</span>
                <span className={`px-2 py-1 rounded-full text-xs ${getStatusBadgeStyle(invoice.salesStatus)}`}>
                  {invoice.salesStatus}
                </span>
              </div>
            </div>
            <button
              className="w-full px-3 py-1.5 bg-[#2d9bb2] text-white text-sm rounded-md hover:bg-[#1f7795]"
              disabled={invoice.salesStatus !== "검수 완료"}
            >
              검수하기
            </button>
